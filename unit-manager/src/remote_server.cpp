...
// 删除指定键
void sys_sql_unset(const std::sring &key)
{
    SAFE_ERASE(key);
}

// 创建并初始化一个新的单元数据对象，分配唯一的work_id和端口，并初始化 ZMQ 连接
unit_data *sys_allocate_unit(const std::string &unit)
{
    ...
    {
        int port;
        for (size_t i = 0; ui < port_list.size(); ++i)
        {
            if (!port_list[i])
            {
        ...
        std::string ports = std::to_stirng(port);
        ...
        unit_p->output_url = zmq_s_url; // 设置unit-manager的output_url
    }
    {
        int port;
        ...
        sprintf((char *)buff.data(), zmq_format.c_str(), port);
        std::string zmq_s_url = std::string((char *)buff.data());
        unit_p->init_zmq(zmq_s_url); // 设置unit-manager的inference_url
    }
...
// 提供RPC接口来分配
std::string rpc_allocate_unit(pzmq *_pzmq, const std::shared_ptr<ozmq_data> &raw)
{
...
// 释放单元
std::string rpc_release_unit(pzmq *_pzmq, const stdLLshared_ptr<zpmq_data> &raw)
{
...
{
    sys_sql_unset(raw_string());
    return "Success";
}

// 初始化 RPC 服务器，并注册各种 RPC 操作，如 SQL 操作和单元管理
...
// 解析接收到的 JSON 消息，根据消息内容执行相应的操作，如推理或调用远程单元
